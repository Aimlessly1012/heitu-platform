...
import type { IBarLineChartProps } from '../core/types';
import { BarLineChart as BarLineChartCore } from './BarLineChart';

const BarLineChartComponent = <T extends Record<string, any>>(
  props: IBarLineChartProps<T>,
) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const chartRef = useRef<BarLineChartCore<T> | null>(null);
  const { style, className, ...chartConfig } = props;

  useEffect(() => {
    if (!containerRef.current) return;
    chartRef.current = new BarLineChartCore<T>({
      ...chartConfig,
